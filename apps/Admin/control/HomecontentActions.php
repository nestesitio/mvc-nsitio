...
class HomecontentActions extends \apps\Core\control\PagesActions {
    ...
    private function setVars($htm){
        $var = \model\querys\HtmVarsQuery::start()->filterByVar('local')->filterByValue('home-quem-somos')->findOne();
        \model\querys\HtmPageHasVarsQuery::start()
                ->filterByHtmId($htm)->filterByHtmVarsId($var->getId())->findOneOrCreate();
